...
# TODO:
# Monday 2:30 PM PPT; Submit Report? Check first day PPT?????????????????????
# Meet Friday @ 9:30PM for Novely @ Backbone (ViT/other) + Proposal something new (see what other people are working)
# Discuss novelty idea from Khin on Friday

# nohup python main_pretraining_novelty.py > nohup_pretraining_novelty.logs && sleep 120 && nohup python main_pretraining.py > nohup_pretraining.logs &
