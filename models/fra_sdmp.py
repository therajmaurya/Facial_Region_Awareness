...
class FRAB_EMAN(nn.Module):
    ...
    def forward(self, im_v1, im_v2=None, **kwargs):
        ...
        # adding SDMP mixing logic
        # print("running with SDMP...")  # TODO: Remove this comment for clean training logs
        lam = np.random.beta(1., 1., size=[im_v1.shape[0], 1, 1, 1]).astype(np.float32)
        lam = torch.from_numpy(lam).cuda()
        im_v1, _ = datamixing(im_v1.clone(), lam)
        im_v2, _ = datamixing(im_v2.clone(), lam)
